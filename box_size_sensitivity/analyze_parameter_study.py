"""Analyse the 5 x 20 square-bed-size sensitivity study."""
...
INPUT_CSV = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(DEFAULT_OUTPUT_FOLDER, "bed_size_sensitivity_results.csv")
)
OUTPUT_FOLDER = sys.argv[2] if len(sys.argv) > 2 else DEFAULT_OUTPUT_FOLDER
PLOT_FOLDER = os.path.join(OUTPUT_FOLDER, "plots")

BASELINE_BED_WIDTH_FRAC_L = 4.0

# Metrics to summarize and plot against the square-bed width W/L.
METRICS = {
...
def compute_bed_size_summary(frame, metrics):
    """Return one descriptive-statistics row per (W/L, metric) pair."""
    rows = []
    for bed_width, group in frame.groupby(
        "boxWidthXYFracL", sort=True
    ):
        group_summary = compute_summary(group, metrics)
        if group_summary.empty:
            continue
        group_summary.insert(0, "boxWidthXYFracL", bed_width)
        rows.append(group_summary)
    if not rows:
        raise RuntimeError("No bed-size-group summary statistics were produced.")
    return pd.concat(rows, ignore_index=True)


def compute_paired_changes_from_baseline(frame, metrics):
    """Summarize within-seed changes relative to the baseline W/L=4.0."""
    ...
    for metric, settings in metrics.items():
        if metric not in frame.columns:
            continue
        data = frame[[
            "spawnSeed", "boxWidthXYFracL", metric
        ]].copy()
        ...
        pivot = data.pivot_table(
            index="spawnSeed", columns="boxWidthXYFracL",
            values=metric, aggfunc="first",
        )
        baseline_candidates = [
            value for value in pivot.columns
            if np.isclose(value, BASELINE_BED_WIDTH_FRAC_L)
        ]
        if not baseline_candidates:
            raise RuntimeError(
                f"The baseline W/L={BASELINE_BED_WIDTH_FRAC_L:g} is "
                "missing from the completed runs."
            )
        baseline_column = baseline_candidates[0]
        for bed_width in sorted(pivot.columns):
            if np.isclose(bed_width, baseline_column):
                baseline_values = pivot[baseline_column].dropna()
                if baseline_values.empty:
                    continue
                differences = pd.Series(0.0, index=baseline_values.index)
            else:
                paired = pivot[[baseline_column, bed_width]].dropna()
                if paired.empty:
                    continue
                differences = paired[bed_width] - paired[baseline_column]
            rows.append({
                "boxWidthXYFracL": bed_width,
                "baselineBoxWidthXYFracL": baseline_column,
                "metric": metric,
                "label": settings["label"],
                "unit": settings["unit"],
...
def _plot_metric_sensitivity_on_axis(ax, frame, metric, settings):
    """Plot matched realizations and group mean +/- SD versus W/L."""
    required = {"boxWidthXYFracL", metric}
    if not required.issubset(frame.columns):
        return False

    columns = ["boxWidthXYFracL", metric]
    ...
    data = data.dropna(subset=["boxWidthXYFracL", metric])
    if data.empty:
        return False

    # The light connecting lines show the common-random-number pairing. Each
    # line corresponds to one shared spawn seed across the five bed widths.
    if "spawnSeed" in data.columns:
        ...
        for _, seed_group in data.dropna(subset=["spawnSeed"]).groupby(
            ...
            seed_group = seed_group.sort_values("boxWidthXYFracL")
            ax.plot(
                seed_group["boxWidthXYFracL"], seed_group[metric],
                color="0.70", linewidth=0.7, alpha=0.25, zorder=1,
            )

    ax.scatter(
        data["boxWidthXYFracL"], data[metric],
        facecolors="none", edgecolors="0.52", s=28, linewidths=0.7,
        alpha=0.65, label="Individual matched realizations", zorder=2,
    )
    grouped = data.groupby("boxWidthXYFracL", sort=True)[metric]
    summary = grouped.agg(["count", "mean", "std"]).reset_index()
    summary["std"] = summary["std"].fillna(0.0)
    ax.errorbar(
        summary["boxWidthXYFracL"], summary["mean"],
    ...
    ax.axvline(
        BASELINE_BED_WIDTH_FRAC_L, color="#D55E00",
        linestyle="--", linewidth=1.2,
        label=f"Baseline W/L = {BASELINE_BED_WIDTH_FRAC_L:g}", zorder=0,
    )
    ...
    ax.set_xlabel("Square-bed width normalized by rock size, W/L [-]")
    ax.set_xticks(sorted(data["boxWidthXYFracL"].unique()))
...
def plot_metric_sensitivity(frame, metric, settings):
    ...
    outpath = os.path.join(PLOT_FOLDER, stem + "_vs_bed_width.png")
...
def plot_key_metric_overview(frame):
    """Put the four primary bed-size-sensitivity responses in one figure."""
    selected = [
    ...
    completed_counts = frame.groupby("boxWidthXYFracL").size()
    ...
    fig.suptitle(f"Square-bed-size sensitivity: {count_text}")
    fig.tight_layout(rect=(0.0, 0.06, 1.0, 0.96))
    outpath = os.path.join(PLOT_FOLDER, "bed_size_sensitivity_key_metrics.png")
...
def plot_q4_q6_reference_comparisons(frame):
    ...
    One figure is produced for each square-bed width. Each gravity point
    is a realization-level mean. Panel A retains all pyscal neighbour methods;
    Panel B uses SANN and translates that bed-size group's mean to the origin.
    """
    required_columns = {
        settings[component]
    ...
    if "boxWidthXYFracL" not in data.columns:
        print(
            "Skipping q4/q6 reference comparison; "
            "boxWidthXYFracL is missing."
        )
        return
    data["boxWidthXYFracL"] = pd.to_numeric(
        data["boxWidthXYFracL"], errors="coerce"
    )
    groups = list(data.dropna(subset=["boxWidthXYFracL"]).groupby(
        "boxWidthXYFracL", sort=True
    ))
    if not groups:
        print("Skipping q4/q6 reference comparison; no bed-size groups found.")
        return
    ...
    for bed_width, subset in groups:
        ...
        figure_title = (
            ...
            f"(square-bed width $W/L={bed_width:g}$)"
        )
        filename_suffix = f"width_{bed_width:g}L".replace(".", "p")
    ...
    if distance_rows:
        ...
        distance_path = os.path.join(
            OUTPUT_FOLDER, "q4_q6_reference_distances_by_bed_width.csv"
        )
...
# Create one structural-reference figure per square-bed width.
...
# One summary row for every (bed width, metric) combination.
summary = compute_bed_size_summary(df, METRICS)
summary_path = os.path.join(
    OUTPUT_FOLDER, "bed_size_sensitivity_summary_statistics.csv"
)
summary.to_csv(summary_path, index=False)

print("\nComputed bed-size-group summary statistics.")
print(summary)
...
if not paired_changes.empty:
    paired_path = os.path.join(
        OUTPUT_FOLDER,
        "bed_size_sensitivity_paired_changes_from_width_4L.csv",
    )
