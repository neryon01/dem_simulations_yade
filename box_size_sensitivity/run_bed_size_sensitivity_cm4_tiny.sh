...
echo "5 x 20 square-bed-size sensitivity launcher started in ${MODE} mode on host $(hostname)."
echo "Root: ${ROOT}"
...
if [[ "${MODE}" == "--preflight" ]]; then
    ...
    echo "Study design: square-bed full widths W/L = 3.5, 4.0, 4.5, 5.0, 5.5."
    echo "Equivalent x/y half-extents / L = 1.75, 2.00, 2.25, 2.50, 2.75."
    echo "Each value uses the same 20 spawn seeds; 100 simulations total."
    echo "Only x/y bed size varies; z geometry and material parameters remain fixed."
...
date
echo "Square-bed-size result collection completed with all currently usable runs."
