...
def plot_q4_q6_reference_comparisons(frame):
    ...
    One two-panel figure is produced for each bed size so distinct numbers of
    rocks are never pooled into one artificial point.
    Panel A retains all pyscal neighbour methods; Panel B uses SANN only.
    """
    required_columns = {
        settings[component]
    ...
    if "nRocksTarget" in data.columns:
        data["nRocksTarget"] = pd.to_numeric(
            data["nRocksTarget"], errors="coerce"
        )
        groups = list(data.dropna(subset=["nRocksTarget"]).groupby(
            "nRocksTarget", sort=True
        ))
    else:
        groups = [(None, data)]

    if not groups:
        print("Skipping q4/q6 reference comparison; no rock-count groups found.")
        return
    ...
    for rock_count, subset in groups:
        fig, (absolute_ax, vector_ax) = plt.subplots(1, 2, figsize=(14.2, 6.2))
        ...
        absolute_ax.legend(fontsize=8, loc="lower right")
        ...
        vector_ax.legend(fontsize=8, loc="lower right")

        if rock_count is None:
            figure_title = "Gravity packing versus ideal q4/q6 references"
            filename_suffix = "gravity"
        else:
            figure_title = (
...
# This is additive: if the rock-count-study results contain pyscal q4/q6
# columns, create the future structural-reference figures. Older result CSVs
# without those columns continue through the original analysis unchanged.
