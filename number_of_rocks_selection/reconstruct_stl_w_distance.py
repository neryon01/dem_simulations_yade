...
try:
    ...
    # q4/q6 is an additional diagnostic and must not invalidate porosity or
    # divisor comparison if pyscal is unavailable in a particular runtime.
    pyscal_q_by_method = {
        method: {
            "q4": np.full(len(accepted_centres), np.nan),
            "q6": np.full(len(accepted_centres), np.nan),
        }
        for method in NEIGHBOR_METHODS
    }
    pyscal_status = "failed"
    pyscal_error = str(error)
    print("WARNING: pyscal q4/q6 calculation was skipped:", pyscal_error)
