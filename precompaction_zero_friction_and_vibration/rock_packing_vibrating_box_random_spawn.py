...
# The Slurm launcher uses this mode to test the exact embedded YADE Python
# runtime before any batch job is submitted.
if os.environ.get("YADE_IMPORT_PREFLIGHT_ONLY", "0") == "1":
    print(
        "YADE runtime import preflight passed: pandas {} / NumPy {}".format(
...
# ============================================================
# Defaults define the Zhao-style frictionless gravity case:
...
utils.readParamsFromTable(
    ...
    # Hertz-Mindlin materials. Only rock-rock contacts are frictionless.
    # Rock-wall contacts retain the wall coefficient so the bed can settle
    # against the container. FrictMat expects an angle [rad], so atan(mu) is
    # used below and the contact-pair values are assigned explicitly.
    rockYoung=20.0e9,
    rockPoisson=0.20,
    ...
    enableVibration=1,             # 0/1; default basic case = off
    vibrationAxis="x",            # "x" or "z"
    vibAmplitudeFrac=0.008,          # amplitude / box size along vibration axis
...
# YADE's default Hertz-Mindlin material mixer takes the smaller of the two
# material friction angles. With frictionless rocks, that default would also
# make rock-wall contacts frictionless. Define every material pair explicitly:
# rock-rock mu=0, while rock-wall and (defensively) wall-wall use wall mu.
rock_rock_friction_angle = 0.0
rock_wall_friction_angle = math.atan(BOX_FRICTION_COEFFICIENT)
contact_friction_by_pair = MatchMaker(
    matches=(
        (matId_rock, matId_rock, rock_rock_friction_angle),
        (matId_rock, matId_box, rock_wall_friction_angle),
        (matId_box, matId_box, rock_wall_friction_angle),
    )
)
...
contact_ip2 = [
    Ip2_FrictMat_FrictMat_MindlinPhys(frictAngle=contact_friction_by_pair)
]
...
print(
    "Contact-pair friction: rock-rock mu = 0.0; rock-wall mu =",
    BOX_FRICTION_COEFFICIENT,
)
...
def write_yade_metrics(
    ...
    metrics = pd.DataFrame([{
        ...
        "rockRockContactFrictionCoefficient": 0.0,
        "rockWallContactFrictionCoefficient": BOX_FRICTION_COEFFICIENT,
