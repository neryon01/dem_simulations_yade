"""Analyse completed compressed-bed wall-relaxation realizations."""
...
# statistics_outputs.
DEFAULT_OUTPUT_FOLDER = "relaxation_statistics_outputs"
INPUT_CSV = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(DEFAULT_OUTPUT_FOLDER, "compression_relaxation_results.csv")
)
...
SOURCE_GRAVITY_RUNS_DIR = Path(
    os.environ.get(
        "GRAVITY_SOURCE_RUNS_DIR",
        PROJECT_ROOT / "gravity_source_runs",
    )
).resolve()
GRAVITY_COMPARISON_CSV = os.path.join(
    OUTPUT_FOLDER, "gravity_to_relaxed_porosity_changes.csv"
)
...
# relaxed packing has a variable local coordination number.
...
def add_matched_gravity_porosity(frame):
    """Read gravity porosity for each matching run_NNN realization."""
    result = frame.copy()
    if "run_id" not in result.columns:
        raise RuntimeError(
            "The relaxation results need run_id to match gravity run_NNN folders."
        )
    if not SOURCE_GRAVITY_RUNS_DIR.is_dir():
        raise RuntimeError(
            "Gravity source directory is missing: {}"
            .format(SOURCE_GRAVITY_RUNS_DIR)
        )
    ...
    values = {target: [] for target in gravity_columns.values()}
    gravity_run_ids = []

    for _, row in result.iterrows():
        candidates = (
            row.get("upstreamGravityRunId"),
        ...
        for candidate in candidates:
            candidate = pd.to_numeric(
                pd.Series([candidate]), errors="coerce"
            ).iloc[0]
            if pd.notna(candidate):
                gravity_run_id = int(candidate)
                break
        if gravity_run_id is None:
            raise RuntimeError(
                "Could not determine the matched gravity run for relaxation "
                "run_id={}.".format(row.get("run_id"))
            )

        porosity_path = (
            SOURCE_GRAVITY_RUNS_DIR
            / f"run_{gravity_run_id:03d}"
            / "porosity_result.csv"
        )
        if not porosity_path.is_file():
            raise RuntimeError(
                "Missing matched gravity porosity file: {}"
                .format(porosity_path)
            )
        gravity = pd.read_csv(porosity_path)
        if len(gravity) != 1:
            raise RuntimeError(
                "{} must contain exactly one row.".format(porosity_path)
            )

        gravity_run_ids.append(gravity_run_id)
        gravity_row = gravity.iloc[0]
        for source, target in gravity_columns.items():
            values[target].append(
                pd.to_numeric(
                    pd.Series([gravity_row.get(source)]), errors="coerce"
                ).iloc[0]
            )

    result["gravity_run_id"] = gravity_run_ids
    for target, column_values in values.items():
        result[target] = column_values

    result["porosityChangeRelaxedMinusGravity"] = (
        numeric_series(result, "porosity")
        - numeric_series(result, "sourceGravityPorosity")
    )
    result["porosityReductionGravityMinusRelaxed"] = (
        numeric_series(result, "sourceGravityPorosity")
        - numeric_series(result, "porosity")
    )
    gravity_porosity = numeric_series(result, "sourceGravityPorosity")
    result["porosityChangePercentOfGravity"] = np.where(
        gravity_porosity.ne(0.0),
        100.0
        * result["porosityChangeRelaxedMinusGravity"]
        / gravity_porosity,
        np.nan,
    ...
    print(
        "Matched gravity porosity from:",
        SOURCE_GRAVITY_RUNS_DIR,
    )
    return result


def relaxation_method_label(frame):
    ...
    if "simulationMode" in frame.columns:
        ...
        if len(modes) == 1:
            ...
            if "wall_relaxation" in mode:
                reductions = numeric_series(
                    frame, "sourceCompressionWidthReductionFrac"
                ).dropna()
                if not reductions.empty:
                    return (
                        "wall relaxation to the original width after "
                        rf"{100.0 * float(reductions.iloc[0]):.0f}\% compression"
                    )
                return "wall relaxation to the original width"
    return "compression relaxation"


def plot_paired_porosity_change(frame):
    """Compare every relaxed bed directly with its matched gravity source."""
    ...
    method = relaxation_method_label(paired)
    ...
    comparison_ax.set_ylabel("Porosity after wall relaxation [-]")
    ...
    bins = min(14, max(6, int(np.sqrt(len(increase)))))
    change_ax.hist(
        increase,
        bins=bins,
        color="#D55E00",
    ...
    change_ax.axvline(
        float(np.mean(increase)),
        color="#0072B2",
        linewidth=2.0,
        label=f"Mean = {float(np.mean(increase)):.4f}",
    )
    change_ax.set_xlabel(
        r"Porosity change $\varepsilon_{relaxed}-\varepsilon_{gravity}$ [-]"
    )
    ...
    fig.suptitle(
        f"Gravity source versus {method} ({len(paired)} paired realizations)",
        fontsize=13,
    )
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.95))
    output_path = os.path.join(
        PLOT_FOLDER, "paired_gravity_to_relaxed_porosity.png"
    )
    ...
    comparison_columns = [
        column for column in (
            ...
            "porosity",
            "porosityChangeRelaxedMinusGravity",
            "porosityReductionGravityMinusRelaxed",
            "porosityChangePercentOfGravity",
        )
        if column in paired.columns
    ]
    paired[comparison_columns].to_csv(GRAVITY_COMPARISON_CSV, index=False)
    print("Saved:", GRAVITY_COMPARISON_CSV)

    obsolete_path = os.path.join(
        PLOT_FOLDER, "paired_compressed_to_relaxed_porosity.png"
    )
    if os.path.isfile(obsolete_path):
        os.remove(obsolete_path)
        print("Removed obsolete compressed-source comparison:", obsolete_path)
    ...
    for rock_count, subset in groups:
        ...
        if sann_data.empty:
            ...
            vector_ax.scatter(
                ...
                label=f"Relaxed mean ({Q_VECTOR_METHOD})", zorder=6,
            )
        ...
        vector_ax.set_title(
            f"B  References relative to relaxed packing ({Q_VECTOR_METHOD})"
        )
        ...
        if rock_count is None:
            figure_title = "Relaxed packing versus ideal q4/q6 references"
            filename_suffix = "relaxed"
        else:
            figure_title = (
                "Relaxed packing versus ideal q4/q6 references "
...
df = add_matched_gravity_porosity(df)
...
summary_path = os.path.join(OUTPUT_FOLDER, "compression_relaxation_summary_statistics.csv")
