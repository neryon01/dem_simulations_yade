...
OUT = sys.argv[2] if len(sys.argv) > 2 else "relaxation_statistics_outputs"
INP = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(OUT, "compression_relaxation_porosity_profiles.csv")
)
...
if summaries:
    ...
    summary_path = os.path.join(
        OUT, "compression_relaxation_porosity_profile_summary.csv"
    )
