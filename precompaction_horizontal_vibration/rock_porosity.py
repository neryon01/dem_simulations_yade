...
if wall_distance_max <= 0:
    ...
    # The folded nearest-wall data remain available as a numerical diagnostic.
    # The paper-facing figure is produced below from two complete wall-to-wall
    # centrelines, after the axially averaged x-y porosity field is available.
...
# Zone 1 and Zone 2 use the lateral coordinates associated with their own
# geometries.  Both coordinates are dimensionless, but they must not be read
# as one continuous physical path through the box:
#
#   Zone 1: delta = nearest-side-wall distance / dp
#   Zone 2: delta = inward diagonal distance from a corner / dp
#
# Values are pooled over all symmetry-equivalent walls and corners.
...
plt.xlabel(r"Relative lateral distance, $\delta$ [-]")
plt.ylabel("Local porosity [-]")
plt.title(
    "Local porosity profiles by lateral zone" + np_title_suffix + "\n"
    r"Zone 1: $\delta=d_{\mathrm{nearest\ wall}}/d_p$; "
    r"Zone 2: $\delta=d_{\mathrm{inward\ diagonal\ from\ corner}}/d_p$",
    fontsize=10,
)
plt.ylim(0.0, 1.0)
plt.legend(
...
# Remove the superseded one-path plot if an existing run directory is
# post-processed again with this version.
obsolete_single_diagonal_plot = (
    PLOT_FOLDER / f"single_complete_diagonal_porosity_{RUN_LABEL}.png"
)
if obsolete_single_diagonal_plot.is_file():
    obsolete_single_diagonal_plot.unlink()
...
# ============================================================
# COMPLETE WALL-TO-WALL CENTRELINE POROSITY PROFILE
# ============================================================
# Sample the two symmetry-equivalent complete centrelines of the axially
# averaged lateral field. Unlike nearest-wall distance, this coordinate is not
# folded at the centre: it runs continuously from one wall to the opposite
# wall, and therefore spans 0 <= d/dp <= Np.
centreline_sample_spacing = min(voxel_size_x, voxel_size_y)
n_centreline_points = max(
    2,
    int(np.ceil(channel_side_length / centreline_sample_spacing)) + 1,
)
centreline_fraction = np.linspace(0.0, 1.0, n_centreline_points)
centreline_distance_from_starting_wall = (
    centreline_fraction * channel_side_length
)
centreline_delta = (
    centreline_distance_from_starting_wall / hamzah_dp
)

centreline_values = {
    "leftToRight": bilinear_sample(
        lateral_porosity_field,
        xmin + centreline_fraction * box_side_x,
        np.full(n_centreline_points, box_center_y),
    ),
    "bottomToTop": bilinear_sample(
        lateral_porosity_field,
        np.full(n_centreline_points, box_center_x),
        ymin + centreline_fraction * box_side_y,
    ),
}
centreline_stack = np.vstack([
    centreline_values[name] for name in ("leftToRight", "bottomToTop")
])
centreline_mean = np.mean(centreline_stack, axis=0)
centreline_std = np.std(centreline_stack, axis=0, ddof=0)
centreline_minimum = np.min(centreline_stack, axis=0)
centreline_maximum = np.max(centreline_stack, axis=0)

wall_to_wall_df = pd.DataFrame({
    "sampleIndex": np.arange(n_centreline_points, dtype=int),
    "centrelineFractionWallToWall": centreline_fraction,
    "distanceFromStartingWall": centreline_distance_from_starting_wall,
    "distanceFromStartingWallByParticleDiameter": centreline_delta,
    "leftToRightPorosity": centreline_values["leftToRight"],
    "bottomToTopPorosity": centreline_values["bottomToTop"],
    "meanAcrossTwoCentrelines": centreline_mean,
    "stdAcrossTwoCentrelines": centreline_std,
    "minimumAcrossTwoCentrelines": centreline_minimum,
    "maximumAcrossTwoCentrelines": centreline_maximum,
    "characteristicParticleDiameter": hamzah_dp,
    "channelToParticleSizeRatioNp": hamzah_np,
    "axialZMin": global_zmin,
    "axialZMax": global_zmax,
    "samplingMethod": (
        "two_complete_orthogonal_centrelines_bilinearly_sampled_from_"
        "unsmoothed_axially_averaged_voxel_field"
    ),
})

plt.figure(figsize=(7.5, 5.0))
for name in ("leftToRight", "bottomToTop"):
    plt.plot(
        centreline_delta,
        centreline_values[name],
        color="0.75",
        linewidth=0.9,
        alpha=0.85,
        label=(
            "Individual wall-to-wall centrelines"
            if name == "leftToRight" else "_nolegend_"
        ),
    )
plt.fill_between(
    centreline_delta,
    ...
    color="C0",
    alpha=0.18,
    label="Minimum-maximum across two centrelines",
)
plt.plot(
    centreline_delta,
    centreline_mean,
    color="C0",
    linewidth=2.4,
    label="Mean across two centrelines",
)
plt.xlabel(
...
plt.title(
    "Wall-to-wall centreline porosity profile" + np_title_suffix + "\n"
    f"Axial average over z = [{global_zmin:.3f}, {global_zmax:.3f}] m",
    fontsize=11,
)
plt.xlim(0.0, hamzah_np)
plt.ylim(0.0, 1.0)
plt.legend(
    loc="upper center",
...
plt.tight_layout()
wall_to_wall_plot_path = (
    PLOT_FOLDER / f"wall_to_wall_centreline_porosity_{RUN_LABEL}.png"
)
plt.savefig(wall_to_wall_plot_path, dpi=200, bbox_inches="tight")
plt.close()
print("Saved:", wall_to_wall_plot_path)

# Remove the former folded nearest-wall figure when an existing run is
# post-processed again.
obsolete_wall_distance_plot = (
    PLOT_FOLDER / f"radial_porosity_profile_{RUN_LABEL}.png"
)
if obsolete_wall_distance_plot.is_file():
    obsolete_wall_distance_plot.unlink()
