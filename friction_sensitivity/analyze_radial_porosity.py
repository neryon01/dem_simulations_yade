"""Compare spatial porosity profiles across rock-friction coefficients."""
...
INP = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(OUT, "friction_sensitivity_porosity_profiles.csv")
)
data = pd.read_csv(INP)
required_columns = {"run_id", "profileType", "rockFrictionCoefficient"}
...
data = data.dropna(subset=["rockFrictionCoefficient"]).copy()
...
def friction_color(value):
    for friction_value, color in FRICTION_COLORS.items():
        if np.isclose(value, friction_value):
            return color
    return None
...
def record(kind, coordinate_name, friction_value, x, mean, std, count):
...
def plot_grouped_profiles(
    ...
    plotted = False
    for friction_value, friction_group in frame.groupby(
        "rockFrictionCoefficient", sort=True
    ):
        x, mean, std, count = aggregate(
            friction_group, coordinate, value, points
        )
        if x_transform is not None:
            x = x_transform(friction_group, x)
        record(kind, coordinate_name, friction_value, x, mean, std, count)
        color = friction_color(friction_value)
        linewidth = 2.8 if np.isclose(friction_value, 0.45) else 2.0
        ax.plot(
            x, mean, color=color, linewidth=linewidth,
            label=f"mu = {friction_value:g} (n={count})",
        )
        ax.fill_between(
...
def complete_diagonal_profiles(frame):
    ...
    for run_id, group in frame.groupby("run_id"):
        ...
        for column in (
            "rockFrictionCoefficient", "spawnSeed", "axialZMin", "axialZMax",
...
if not wall_to_wall.empty:
    ...
    plot_grouped_profiles(
        ...
        "Wall-to-wall porosity sensitivity\n" + axial_interval_text(wall_to_wall),
        "wall_to_wall_porosity_vs_friction.png",
...
if not vertical.empty:
    plot_grouped_profiles(
        ...
        "Vertical porosity sensitivity",
        "vertical_porosity_vs_friction.png",
        x_limits=lambda frame: (0.0, 1.0),
    )

    if "corePorosity" in vertical.columns:
        for friction_value, friction_group in vertical.groupby(
            "rockFrictionCoefficient", sort=True
        ):
            x, mean, std, count = aggregate(
                friction_group, "zNormByBedHeight", "corePorosity", 60
            )
            record(
                "vertical_core", "(z-zFloor)/H", friction_value,
                x, mean, std, count,
            )
...
if not zones.empty:
    zone_summary = zones.groupby(
        ["rockFrictionCoefficient", "zoneId", "zoneName"], as_index=False
    ...
    summaries.append(pd.DataFrame({
        ...
        "rockFrictionCoefficient": zone_summary["rockFrictionCoefficient"],
    ...
    for friction_value, group in zone_summary.groupby(
        "rockFrictionCoefficient", sort=True
    ):
        group = group.sort_values("zoneId")
        ax.errorbar(
            group["zoneId"], group["mean"], yerr=group["std"],
            color=friction_color(friction_value), marker="o", capsize=3,
            linewidth=2.5 if np.isclose(friction_value, 0.45) else 1.8,
            label=f"mu = {friction_value:g}",
        )
    labels = (
    ...
    ax.set_title("Lateral-zone porosity sensitivity" + np_title_suffix(zones))
    ax.set_ylim(0.0, 1.0)
    ...
    fig.tight_layout()
    output_path = os.path.join(plots, "lateral_porosity_zones_vs_friction.png")
    fig.savefig(output_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print("Saved:", output_path)
...
if not diagonal.empty:
    ...
    plot_grouped_profiles(
        ...
        "Corner-to-corner porosity sensitivity\n" + axial_interval_text(diagonal),
        "diagonal_porosity_vs_friction.png",
    )
...
if summaries:
    ...
    summary_path = os.path.join(
        OUT, "friction_sensitivity_porosity_profile_summary.csv"
    )
    summary.to_csv(summary_path, index=False)
    print("Saved:", summary_path)
print("Saved friction-resolved spatial-porosity plots in:", plots)
