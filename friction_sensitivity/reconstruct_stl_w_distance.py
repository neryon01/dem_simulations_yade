...
# The pairwise STL intersection/distance pass is far too expensive for a
# multi-run sensitivity recovery and is not needed for porosity or q4/q6.
# Disabling it also avoids vtkIntersectionPolyDataFilter stalls. Metrics that
# require that pass are written as NaN instead of being reported as zero.
PAIRWISE_STL_DIAGNOSTICS_ENABLED = False

# ------------------------------------------------------------
# The former pile-connectivity filter is retained only as an MST diagnostic.
# It must not decide which rocks belong to the bed: YADE clumps can be in
# contact while their reconstructed STL surfaces have small gaps, and signed
# STL-distance jumps can split a physically valid packing. Bed membership is
# therefore defined exclusively by YADE: final poses minus deleted_rocks.csv.
# ------------------------------------------------------------
...
for _, row in final.iterrows():
    ...
    reconstructed.append({
        ...
        "mesh": mesh,
        "distanceSampleMesh": (
            densify_mesh_for_distance(mesh, DISTANCE_SAMPLING_SUBDIVISIONS)
            if PAIRWISE_STL_DIAGNOSTICS_ENABLED else None
        ),
...
pair_iterator = (
    itertools.combinations(reconstructed, 2)
    if PAIRWISE_STL_DIAGNOSTICS_ENABLED else ()
)
for rock_a, rock_b in pair_iterator:
...
results_df = pd.DataFrame(results, columns=pairwise_columns)
...
if not PAIRWISE_STL_DIAGNOSTICS_ENABLED:
    print("Pairwise reconstructed-STL diagnostics: disabled for recovery")
...
# ------------------------------------------------------------
# DIAGNOSTIC CONNECTIVITY VIA MINIMUM SPANNING TREE
#
...
# threshold could.
#
# The MST's edge weights provide a useful indication of apparent STL-surface
# separation. They are deliberately not used for acceptance because signed
# distances can contain a larger jump inside a valid packed bed.
# ------------------------------------------------------------
...
if len(mst_weights) > 0:
    ...
    if len(gaps) > 0:
        ...
        if max_gap > MIN_SIGNIFICANT_GAP:
            ...
            print(f"-> Diagnostic MST cut at weight >= {cut_weight:.4g}.")
        else:
...
nearest_distance_by_clump = defaultdict(
    lambda: np.nan if not PAIRWISE_STL_DIAGNOSTICS_ENABLED else float("inf")
)
for row in results:
...
# ============================================================
# Contacts are evaluated among every YADE-retained rock. STL overlap and MST
# membership remain separate diagnostics and do not alter the packing.
...
particle_count_values = np.asarray(list(particle_contact_counts.values()), dtype=float)
finite_particle_counts = particle_count_values[np.isfinite(particle_count_values)]
min_contacts_per_rock = (
    float(np.min(finite_particle_counts)) if len(finite_particle_counts) else float("nan")
)
max_contacts_per_rock = (
    float(np.max(finite_particle_counts)) if len(finite_particle_counts) else float("nan")
)
std_contacts_per_rock = (
    float(np.std(finite_particle_counts, ddof=1))
    if len(finite_particle_counts) > 1 else float("nan")
)
...
n_stl_collisions = (
    int((results_df["status"] == "collision").sum())
    if PAIRWISE_STL_DIAGNOSTICS_ENABLED else float("nan")
)
n_no_collision = (
    int((results_df["status"] == "no_collision").sum())
    if PAIRWISE_STL_DIAGNOSTICS_ENABLED else float("nan")
)
...
summary = pd.DataFrame([{
    "stlProcessingMode": (
        "pairwise_stl_diagnostics"
        if PAIRWISE_STL_DIAGNOSTICS_ENABLED
        else "retained_clumps_without_pairwise_stl_diagnostics"
    ),
    ...
    "distanceSamplingSubdivisions": (
        DISTANCE_SAMPLING_SUBDIVISIONS
        if PAIRWISE_STL_DIAGNOSTICS_ENABLED else 0
    ),
