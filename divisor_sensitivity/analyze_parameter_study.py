"""Analyse the 4 x 15 common clump-divisor sensitivity study."""
...
INPUT_CSV = (
    sys.argv[1]
    if len(sys.argv) > 1
    else os.path.join(DEFAULT_OUTPUT_FOLDER, "divisor_sensitivity_results.csv")
)
OUTPUT_FOLDER = sys.argv[2] if len(sys.argv) > 2 else DEFAULT_OUTPUT_FOLDER
PLOT_FOLDER = os.path.join(OUTPUT_FOLDER, "plots")

# Metrics to summarize and plot against the common clump-resolution divisor.
METRICS = {
    ...
    "totalTemplateSphereCount": {
        "label": "Total spheres across the four clump templates",
        "xlabel": "Total template-member sphere count [-]",
        "filename": "total_template_sphere_count_distribution.png",
        "unit": "-",
    },
    "meanTemplateSphereCount": {
        "label": "Mean spheres per clump template",
        "xlabel": "Mean template-member sphere count [-]",
        "filename": "mean_template_sphere_count_distribution.png",
        "unit": "-",
    },
}
...
def compute_divisor_summary(frame, metrics):
    """Return one descriptive-statistics row per (divisor, metric) pair."""
    rows = []
    for divisor, group in frame.groupby(
        "rockResolutionDivisor", sort=True
    ):
        group_summary = compute_summary(group, metrics)
        if group_summary.empty:
            continue
        group_summary.insert(0, "rockResolutionDivisor", int(divisor))
        rows.append(group_summary)
    if not rows:
        raise RuntimeError("No divisor-group summary statistics were produced.")
    return pd.concat(rows, ignore_index=True)


def compute_adjacent_paired_changes(frame, metrics):
    """Summarize within-seed changes between adjacent tested divisors."""
    if "spawnSeed" not in frame.columns:
        print("spawnSeed is absent; adjacent paired changes cannot be computed.")
        return pd.DataFrame()
    ...
    for metric, settings in metrics.items():
        if metric not in frame.columns:
            continue
        data = frame[[
            "spawnSeed", "rockResolutionDivisor", metric
        ]].copy()
        ...
        pivot = data.pivot_table(
            index="spawnSeed", columns="rockResolutionDivisor",
            values=metric, aggfunc="first",
        )
        divisors = sorted(int(value) for value in pivot.columns)
        for previous_divisor, current_divisor in zip(divisors[:-1], divisors[1:]):
            paired = pivot[[previous_divisor, current_divisor]].dropna()
            if paired.empty:
                continue
            differences = paired[current_divisor] - paired[previous_divisor]
            rows.append({
                "previousRockResolutionDivisor": previous_divisor,
                "currentRockResolutionDivisor": current_divisor,
                "metric": metric,
                "label": settings["label"],
                "unit": settings["unit"],
                "numberOfPairedRealizations": len(differences),
                "meanPairedChange": differences.mean(),
                "stdPairedChange": (
                    differences.std(ddof=1) if len(differences) > 1 else 0.0
                ),
                "minimumPairedChange": differences.min(),
                "maximumPairedChange": differences.max(),
            })
    return pd.DataFrame(rows)


def _plot_metric_sensitivity_on_axis(ax, frame, metric, settings):
    """Plot matched realizations and group mean +/- SD versus divisor."""
    required = {"rockResolutionDivisor", metric}
    if not required.issubset(frame.columns):
        return False

    columns = ["rockResolutionDivisor", metric]
    ...
    data = data.dropna(subset=["rockResolutionDivisor", metric])
    if data.empty:
        return False

    # The light connecting lines show the common-random-number pairing. Each
    # line corresponds to one shared spawn seed across the four divisors.
    if "spawnSeed" in data.columns:
        ...
        for _, seed_group in data.dropna(subset=["spawnSeed"]).groupby(
            ...
            seed_group = seed_group.sort_values("rockResolutionDivisor")
            ax.plot(
                seed_group["rockResolutionDivisor"], seed_group[metric],
                color="0.70", linewidth=0.7, alpha=0.25, zorder=1,
            )

    ax.scatter(
        data["rockResolutionDivisor"], data[metric],
        facecolors="none", edgecolors="0.52", s=28, linewidths=0.7,
        alpha=0.65, label="Individual matched realizations", zorder=2,
    )
    grouped = data.groupby("rockResolutionDivisor", sort=True)[metric]
    summary = grouped.agg(["count", "mean", "std"]).reset_index()
    summary["std"] = summary["std"].fillna(0.0)
    ax.errorbar(
        summary["rockResolutionDivisor"], summary["mean"],
    ...
    ax.set_xlabel("Common clump-resolution divisor, d [-]")
    ax.set_xticks(sorted(data["rockResolutionDivisor"].unique()))
...
def plot_metric_sensitivity(frame, metric, settings):
    ...
    outpath = os.path.join(PLOT_FOLDER, stem + "_vs_divisor.png")
...
def plot_key_metric_overview(frame):
    """Put the four primary divisor-sensitivity responses in one figure."""
    selected = [
    ...
    fig.suptitle("Clump-divisor sensitivity: 15 matched realizations per value")
    fig.tight_layout(rect=(0.0, 0.06, 1.0, 0.96))
    outpath = os.path.join(PLOT_FOLDER, "divisor_sensitivity_key_metrics.png")
...
def plot_q4_q6_reference_comparisons(frame):
    ...
    One figure is produced for each common divisor. Each gravity point
    is a realization-level mean. Panel A retains all pyscal neighbour methods;
    Panel B uses SANN and translates that divisor group's mean to the origin.
    """
    required_columns = {
        settings[component]
    ...
    if "rockResolutionDivisor" not in data.columns:
        print(
            "Skipping q4/q6 reference comparison; "
            "rockResolutionDivisor is missing."
        )
        return
    data["rockResolutionDivisor"] = pd.to_numeric(
        data["rockResolutionDivisor"], errors="coerce"
    )
    groups = list(data.dropna(subset=["rockResolutionDivisor"]).groupby(
        "rockResolutionDivisor", sort=True
    ))
    if not groups:
        print("Skipping q4/q6 reference comparison; no divisor groups found.")
        return
    ...
    for divisor, subset in groups:
        ...
        figure_title = (
            ...
            f"(common divisor $d={int(divisor)}$)"
        )
        filename_suffix = f"divisor_{int(divisor)}"
    ...
    if distance_rows:
        ...
        distance_path = os.path.join(
            OUTPUT_FOLDER, "q4_q6_reference_distances_by_divisor.csv"
        )
...
# Create one structural-reference figure per common divisor.
...
# One summary row for every (common divisor, metric) combination.
summary = compute_divisor_summary(df, METRICS)
summary_path = os.path.join(
    OUTPUT_FOLDER, "divisor_sensitivity_summary_statistics.csv"
)
summary.to_csv(summary_path, index=False)

print("\nComputed divisor-group summary statistics.")
print(summary)
print("Saved:", summary_path)

paired_changes = compute_adjacent_paired_changes(df, METRICS)
if not paired_changes.empty:
    paired_path = os.path.join(
        OUTPUT_FOLDER,
        "divisor_sensitivity_paired_adjacent_changes.csv",
    )
