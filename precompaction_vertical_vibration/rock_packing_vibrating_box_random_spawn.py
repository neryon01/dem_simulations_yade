...
# ============================================================
# Defaults rebuild a previously settled 80-rock gravity bed from its final
# pose CSV.  This package represents ONE precompaction method at a time.
#
# Folder presets:
#   vibx:       enableVibration=1, vibrationAxis="x",
#               vibAmplitudeFrac=0.008, enableCompression=0
#   vibz:       enableVibration=1, vibrationAxis="z",
#               vibAmplitudeFrac=0.004, enableCompression=0
#   compaction: enableVibration=0, enableCompression=1,
#               compressionWidthReductionFrac=0.15
#
# Keep exactly one of enableVibration and enableCompression equal to 1.
#
utils.readParamsFromTable(
    ...
    enableVibration=1,             # default package preset: vibx
    ...
    enableCompression=0,           # set to 1 only in the compaction folder
    compressionWidthReductionFrac=0.15,
...
if int(ENABLE_VIBRATION) + int(ENABLE_COMPRESSION) != 1:
    raise ValueError(
        "Select exactly one precompaction method: enableVibration and "
        "enableCompression must be (1, 0) or (0, 1)."
    )
...
if os.environ.get("YADE_IMPORT_PREFLIGHT_ONLY", "0") == "1":
    print(
        ...
        "enableVibration={}, vibrationAxis={}, vibAmplitudeFrac={}, "
        "enableCompression={}, compressionWidthReductionFrac={}, nRocks={}".format(
            pd.__version__,
            np.__version__,
            ENABLE_VIBRATION,
            VIBRATION_AXIS,
...
SOURCE_RUN_DIR = SOURCE_GRAVITY_RUNS_DIR / f"run_{SOURCE_RUN_ID:03d}"
...
print("Initialization = reconstructed final gravity poses")
...
# ============================================================
# GRAVITY-BED RECONSTRUCTION AND POSE EXPORT
# ============================================================
...
def validate_source_metadata(source_poses):
    source_metrics = pd.read_csv(SOURCE_RUN_DIR / "yade_metrics.csv")
    if len(source_metrics) != 1:
        raise RuntimeError("Source yade_metrics.csv must contain exactly one row.")
    source_metrics = source_metrics.iloc[0]
...
def rebuild_saved_gravity_bed(source_poses):
    """Create clumps directly at the final saved gravity-pose member positions."""
...
sourceGravityMetrics = validate_source_metadata(sourceFinalPoses)
rebuild_saved_gravity_bed(sourceFinalPoses)
...
def simulation_mode_name():
    if ENABLE_VIBRATION and ENABLE_COMPRESSION:
        return "gravity_restart_vibration_then_compression"
    if ENABLE_VIBRATION:
        return "gravity_restart_vibration"
    if ENABLE_COMPRESSION:
        return "gravity_restart_compression"
    return "gravity_restart_relaxation_only"
...
def write_yade_metrics(
    ...
    metrics = pd.DataFrame([{
        ...
        "sourceGravityRunId": SOURCE_RUN_ID,
        "sourceGravityRunDirectory": str(SOURCE_RUN_DIR),
        "initializationMode": "reconstructed_from_final_gravity_poses",
...
def controlSimulation():
    ...
    # This fallback clock starts when the saved bed has been reconstructed.
    # Reaching it is a completed run and follows the normal export path.
    if (
        all_rocks_inserted
