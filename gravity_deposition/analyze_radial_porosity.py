...
INP = sys.argv[1] if len(sys.argv) > 1 else os.path.join(OUT, "gravity_total_porosity_profiles.csv")
...
def complete_wall_to_wall_profiles(frame):
    """Return one full wall-to-wall centreline profile per realization.

    Use directly saved wall-to-wall profiles when available. For the current
    gravity outputs, reconstruct the two orthogonal centrelines from the saved
    axially averaged lateral porosity field and average those centrelines.
    """
    coordinate = "distanceFromStartingWallByParticleDiameter"
    value = "meanAcrossTwoCentrelines"
    direct = frame.loc[frame.profileType.eq("wall_to_wall")].copy()
    if {coordinate, value}.issubset(direct.columns):
        direct[coordinate] = pd.to_numeric(direct[coordinate], errors="coerce")
        direct[value] = pd.to_numeric(direct[value], errors="coerce")
        direct = direct.dropna(subset=["run_id", coordinate, value])
        if not direct.empty:
            return direct, coordinate, value

    lateral = frame.loc[frame.profileType.eq("lateral_field")].copy()
    required = {
        "xNormAcrossBox",
        "yNormAcrossBox",
        "axiallyAveragedPorosity",
        "channelToParticleSizeRatioNp",
    }
    missing = sorted(required - set(lateral.columns))
    if lateral.empty or missing:
        raise RuntimeError(
            "Wall-to-wall plotting requires lateral_field rows containing: "
            + ", ".join(sorted(required))
        )

    rebuilt = []
    for run_id, group in lateral.groupby("run_id"):
        work = group.copy()
        ...
        field_table = work.pivot_table(
            index="xNormAcrossBox",
            columns="yNormAcrossBox",
            values="axiallyAveragedPorosity",
            aggfunc="mean",
        ).sort_index().sort_index(axis=1)
        field_table = field_table.dropna(axis=0, how="all").dropna(
            axis=1, how="all"
        )
        if (
            field_table.shape[0] < 2
            or field_table.shape[1] < 2
            or field_table.isna().any().any()
        ):
            continue

        field = field_table.to_numpy(dtype=float)
        x_centres = field_table.index.to_numpy(dtype=float)
        y_centres = field_table.columns.to_numpy(dtype=float)
        number_of_points = max(80, len(x_centres), len(y_centres))
        fraction = np.linspace(0.0, 1.0, number_of_points)

        porosity_at_mid_y = np.asarray([
            np.interp(0.5, y_centres, row) for row in field
        ])
        left_to_right = np.interp(
            fraction, x_centres, porosity_at_mid_y
        )

        porosity_at_mid_x = np.asarray([
            np.interp(0.5, x_centres, field[:, column_index])
            for column_index in range(field.shape[1])
        ])
        bottom_to_top = np.interp(
            fraction, y_centres, porosity_at_mid_x
        )

        np_ratio = float(work["channelToParticleSizeRatioNp"].iloc[0])
        run_profile = pd.DataFrame({
            "run_id": int(run_id),
            coordinate: fraction * np_ratio,
            "leftToRightPorosity": left_to_right,
            "bottomToTopPorosity": bottom_to_top,
            value: 0.5 * (left_to_right + bottom_to_top),
            "channelToParticleSizeRatioNp": np_ratio,
        })
        for column in ("axialZMin", "axialZMax"):
            if column in work.columns:
                values = pd.to_numeric(work[column], errors="coerce").dropna()
                if not values.empty:
                    run_profile[column] = float(values.iloc[0])
        rebuilt.append(run_profile)

    if not rebuilt:
        raise RuntimeError(
            "No complete lateral fields were available for wall-to-wall plotting."
        )
    return pd.concat(rebuilt, ignore_index=True), coordinate, value
...
if not wall.empty:
    wall_coordinate = "dWallMidByParticleDiameter"
    if wall_coordinate not in wall.columns:
        wall[wall_coordinate] = np.nan
    wall[wall_coordinate] = pd.to_numeric(
        wall[wall_coordinate], errors="coerce"
    )
    missing_coordinate = wall[wall_coordinate].isna()
    if missing_coordinate.any() and {
        "run_id", "dWallMid", "characteristicParticleDiameter"
    }.issubset(data.columns):
        diameter_by_run = (
            data.assign(
                characteristicParticleDiameter=pd.to_numeric(
                    data["characteristicParticleDiameter"], errors="coerce"
                )
            )
            .dropna(subset=["characteristicParticleDiameter"])
            .groupby("run_id")["characteristicParticleDiameter"]
            .first()
        )
        diameter = wall["run_id"].map(diameter_by_run)
        wall.loc[missing_coordinate, wall_coordinate] = (
            pd.to_numeric(
                wall.loc[missing_coordinate, "dWallMid"], errors="coerce"
            )
            / diameter.loc[missing_coordinate]
        )
    x, mean, std, n = aggregate(wall, wall_coordinate, "localPorosity", 60)
    record("wall_distance", "dWall/dp", x, mean, std, n)
    ...
    plt.xlabel("Distance from nearest side wall / characteristic particle diameter [-]")
    plt.ylabel("Local porosity [-]")
    plt.title("Wall-distance porosity profile" + np_title_suffix(wall), fontsize=11)
    plt.legend(loc="upper center",bbox_to_anchor=(.5,-.18),ncol=2,fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(plots,"wall_porosity_profile_aggregate.png"),dpi=200,bbox_inches="tight"); plt.close()

lateral_field = subset("lateral_field")
direct_wall_to_wall = subset("wall_to_wall")
if not lateral_field.empty or not direct_wall_to_wall.empty:
    wall_to_wall, wall_to_wall_coordinate, wall_to_wall_value = (
        complete_wall_to_wall_profiles(data)
    )
    x, mean, std, n = aggregate(
        wall_to_wall,
        wall_to_wall_coordinate,
        wall_to_wall_value,
        80,
    )
    ...
    plt.xlabel(
        "Distance from starting wall / characteristic particle diameter [-]"
    )
    plt.ylabel("Axially averaged local porosity [-]")
    plt.title(
        "Gravity wall-to-wall centreline porosity profile"
    ...
    plt.ylim(0, 1)
    plt.legend(
        loc="upper center", bbox_to_anchor=(.5, -.18), ncol=2, fontsize=8
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(
            plots,
            "wall_to_wall_centerline_porosity_profile_aggregate.png",
        ),
        dpi=200,
        bbox_inches="tight",
    )
...
if summaries:
    ...
    summary_path = os.path.join(OUT, "gravity_total_porosity_profile_summary.csv")
