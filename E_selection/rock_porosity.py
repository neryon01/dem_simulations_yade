...
# Nearest-wall profile bin thickness, as a fraction of L. This is the square-
# box analogue of the cylinder coordinate (R-r)/d_p used in the experiments.
WALL_BIN_WIDTH_FACTOR = 0.1

# Horizontal-slab thickness for the vertical porosity profile, expressed as a
# fraction of L. The profile is evaluated from the box floor to the highest
# accepted reconstructed-STL point. Both the complete box cross-section and
# the lateral-wall-inset core are reported from the same slabs.
...
# and the non-dimensional coordinate is d_wall/L. The wall itself is an
# exact zero-thickness boundary, while voxel bins have finite thickness.
...
if wall_distance_max <= 0:
    ...
    in_range = (wall_distance >= 0.0) & (wall_distance <= wall_edges[-1])
    ...
    print("coordinate = d_wall/L; exact wall anchor = (0, 1)")
    ...
    plt.plot(wall_df["zRMid"], wall_df["localPorosity"], marker="o", linewidth=1.5,
    ...
    plt.axhline(
        interior_metrics["porosity"],
        color="gray",
        linestyle=":",
        linewidth=1,
        label=f"Interior porosity ε = {interior_metrics['porosity']:.3f}",
    )
    plt.xlabel("Non-dimensional distance from nearest wall, d_wall / L [-]")
    plt.ylabel("Local porosity ε [-]")
    plt.title("Wall-distance porosity profile")
    plt.legend()
    plt.tight_layout()
    profile_plot_path = PLOT_FOLDER / f"radial_porosity_profile_{RUN_LABEL}.png"
    plt.savefig(profile_plot_path, dpi=200)
    plt.close()
...
if vertical_profile_height <= 0.0 or vertical_bin_width <= 0.0:
    ...
    plt.plot(
        vertical_df["zNormByBedHeight"],
        vertical_df["corePorosity"],
        marker="s",
        linewidth=1.5,
        label="Side-wall-inset core",
    )
    plt.axhline(
        porosity,
        color="black",
    ...
    plt.axhline(
        ...
        linestyle=":",
        linewidth=1,
        label=f"Interior porosity = {interior_metrics['porosity']:.3f}",
    )
    plt.xlabel("Normalized height above box floor, (z - z_floor) / H [-]")
    plt.ylabel("Local porosity, epsilon(z) [-]")
    ...
    plt.savefig(vertical_plot_path, dpi=200)
    plt.close()
    print("Saved:", vertical_plot_path)


# ============================================================
# HAMZAH-STYLE LATERAL POROSITY ZONES
# ============================================================
...
if positive_rock_volumes.size == 0:
    raise RuntimeError(
        "No positive reconstructed-rock volumes are available for Hamzah "
        "zone normalization."
    )
...
hamzah_dp = float(np.mean(equivalent_particle_diameters))
hamzah_rp = 0.5 * hamzah_dp

box_side_x = xmax - xmin
box_side_y = ymax - ymin
if not np.isclose(box_side_x, box_side_y, rtol=1.0e-6, atol=1.0e-12):
    raise RuntimeError(
        "Hamzah zoning requires a square horizontal box, but the x and y "
        "side lengths differ."
    )
hamzah_side_length = 0.5 * (box_side_x + box_side_y)
hamzah_np = hamzah_side_length / hamzah_dp
...
def hamzah_lateral_coordinates(x_coordinates, y_coordinates):
    """Return zone ids and Hamzah-style normalized lateral coordinates."""
...
# Plot the geometric zoning and measured zone-average porosities together.
zone_plot_x, zone_plot_y = np.meshgrid(xs, ys, indexing="xy")
...
axes[0].set_xlabel("x")
axes[0].set_ylabel("y")
axes[0].set_title("Hamzah-style square-channel zones")
axes[0].legend(
    handles=[
...
axes[1].set_title("Measured reconstructed-STL porosity")
for index, value in enumerate(hamzah_zone_df["porosity"]):
    axes[1].text(index, min(0.98, value + 0.025), f"{value:.3f}", ha="center")
fig.suptitle(
    f"Lateral porosity zones (Np = {hamzah_np:.2f}, dp = {hamzah_dp:.4g})"
)
fig.tight_layout()
hamzah_zone_plot_path = PLOT_FOLDER / f"hamzah_porosity_zones_{RUN_LABEL}.png"
fig.savefig(hamzah_zone_plot_path, dpi=200)
plt.close(fig)
...
plt.title("Hamzah-style lateral porosity profiles")
plt.ylim(0.0, 1.0)
plt.legend()
plt.tight_layout()
hamzah_profile_plot_path = (
    PLOT_FOLDER / f"hamzah_zone_porosity_profiles_{RUN_LABEL}.png"
)
plt.savefig(hamzah_profile_plot_path, dpi=200)
plt.close()
...
#
# The four symmetry-equivalent corner-to-centre diagonals are then sampled
# from this unsmoothed field. Their individual values, mean, standard
# deviation, minimum, and maximum are retained.
...
n_diagonal_points = max(
    2,
    int(np.ceil(corner_to_center_distance / diagonal_sample_spacing)) + 1,
)
diagonal_fraction = np.linspace(0.0, 1.0, n_diagonal_points)
diagonal_distance_from_corner = (
    diagonal_fraction * corner_to_center_distance
)
...
diagonal_values = {}
for diagonal_name, (corner_x, corner_y) in diagonal_corners.items():
    query_x = corner_x + diagonal_fraction * (box_center_x - corner_x)
    query_y = corner_y + diagonal_fraction * (box_center_y - corner_y)
...
diagonal_stack = np.vstack([
    diagonal_values[name] for name in diagonal_corners
])
...
diagonal_profile_df = pd.DataFrame({
    ...
    "diagonalFractionCornerToCenter": diagonal_fraction,
    "distanceFromCorner": diagonal_distance_from_corner,
    "deltaDistanceFromCornerByParticleDiameter": diagonal_delta,
    "bottomLeftPorosity": diagonal_values["bottomLeft"],
    "bottomRightPorosity": diagonal_values["bottomRight"],
    "topLeftPorosity": diagonal_values["topLeft"],
    "topRightPorosity": diagonal_values["topRight"],
    "meanAcrossFourDiagonals": diagonal_mean,
    "stdAcrossFourDiagonals": diagonal_std,
    "minimumAcrossFourDiagonals": diagonal_minimum,
    "maximumAcrossFourDiagonals": diagonal_maximum,
    "characteristicParticleDiameter": hamzah_dp,
    "channelToParticleSizeRatioNp": hamzah_np,
    "axialZMin": global_zmin,
    "axialZMax": global_zmax,
    "samplingMethod": "bilinear_from_unsmoothed_axially_averaged_voxel_field",
})
...
print("maximum diagonal delta =", float(diagonal_delta[-1]))
...
for corner_x, corner_y in diagonal_corners.values():
    ax.plot(
        [corner_x, box_center_x],
        [corner_y, box_center_y],
        color="white",
        linestyle="--",
        linewidth=1.0,
        alpha=0.9,
...
ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_title("Axially averaged lateral porosity field")
...
for diagonal_name in diagonal_corners:
    plt.plot(
        diagonal_delta,
        ...
        linewidth=1.0,
        label=(
            "Individual corner-to-centre diagonals"
            if diagonal_name == "bottomLeft" else None
        ),
    )
plt.fill_between(
    diagonal_delta,
    ...
    color="C0",
    alpha=0.18,
    label="Minimum-maximum across four diagonals",
)
plt.plot(
    diagonal_delta,
    diagonal_mean,
    color="C0",
    linewidth=2.4,
    label="Mean across four diagonals",
)
plt.axvline(
    HAMZAH_CORNER_DELTA_MAX,
    color="#E45756",
...
plt.xlabel("Diagonal distance from corner / characteristic particle diameter, delta [-]")
plt.ylabel("Axially averaged local porosity [-]")
plt.title("Corner-to-centre diagonal porosity profile")
plt.ylim(0.0, 1.0)
...
plt.savefig(diagonal_plot_path, dpi=200)
plt.close()
...
colorbar = fig.colorbar(contour_xz, ax=axes, fraction=0.035, pad=0.04)
colorbar.set_label("Local porosity [-]")
fig.suptitle("Central vertical porosity contours")
fig.subplots_adjust(left=0.08, right=0.90, bottom=0.15, top=0.86, wspace=0.18)
...
result = pd.DataFrame([{
    ...
    "wallBinWidth": WALL_BIN_WIDTH_FACTOR * L,
    "wallBoundaryPorosity": 1.0,
    "verticalBinWidthFactor": VERTICAL_BIN_WIDTH_FACTOR,
    "verticalBinWidth": VERTICAL_BIN_WIDTH_FACTOR * L,
