...
utils.readParamsFromTable(
    ...
    # With 60 insertions, every rock type is used exactly 15 times.
    ...
    insertionPeriodSeconds=0.6,   # physical simulation time between insertions [s]

    # Box geometry. geom.facetBox uses HALF-extents. x and y deliberately
    # share one setting; z can be adjusted independently.
    boxHalfExtentXYFracL=2,      # box half-width / L in x and y
    ...
    rockYoung=30.0e9,
    rockPoisson=0.20,
    rockFrictionCoefficient=0.45,  # intermediate Suhr case: 0.01, 0.45, 0.8
    rockDensity=2660.0,
    boxYoung=200.0e9,
    boxPoisson=0.28,
    ...
    newtonDamping=0.4,
    timestepSafety=0.25,
    vtkExportPeriod=200000,          # <= 0 disables periodic VTK; final frame remains
...
# Final geometry-screening selections. The lattice rotation changes the
# sphere arrangement relative to the rock; it is not an initial orientation
# of the whole falling rock.
