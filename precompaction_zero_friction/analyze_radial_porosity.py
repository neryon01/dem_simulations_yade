...
INP = sys.argv[1] if len(sys.argv) > 1 else os.path.join(OUT, "zhao_frictionless_porosity_profiles.csv")
...
def complete_wall_to_wall_profiles(frame, coordinate, value):
    """Mirror the nearest-wall profile to reconstruct a full wall-to-wall curve.

    ``rock_porosity.py`` pools the square cross-section by distance to the
    nearest wall, so its stored coordinate naturally ends at the centreline.
    The opposite half contains the same pooled bins in reverse order.  This
    transformation changes only the plotting coordinate; no porosity values
    are recalculated or invented.
    """
    rebuilt = []
    output_coordinate = "wallToWallPositionByParticleDiameter"

    for run_id, group in frame.groupby("run_id"):
        group = group.dropna(subset=[coordinate, value]).copy()
        group[coordinate] = pd.to_numeric(group[coordinate], errors="coerce")
        group[value] = pd.to_numeric(group[value], errors="coerce")
        group = group.dropna(subset=[coordinate, value]).sort_values(coordinate)
        group = group.drop_duplicates(subset=[coordinate], keep="first")
        if len(group) < 2:
            continue

        half_x = group[coordinate].to_numpy(dtype=float)
        half_porosity = group[value].to_numpy(dtype=float)

        width = np.nan
        if "channelToParticleSizeRatioNp" in group.columns:
            widths = pd.to_numeric(
                group["channelToParticleSizeRatioNp"], errors="coerce"
            ).dropna()
            if not widths.empty:
                width = float(widths.median())

        if (
            not np.isfinite(width)
            or width < 2.0 * half_x[-1] - 1.0e-10
        ):
            positive_steps = np.diff(half_x)
            positive_steps = positive_steps[positive_steps > 0.0]
            half_step = (
                0.5 * float(np.median(positive_steps))
                if len(positive_steps) else 0.0
            )
            width = 2.0 * (float(half_x[-1]) + half_step)

        full_x = np.concatenate([half_x, width - half_x[::-1]])
        full_porosity = np.concatenate([half_porosity, half_porosity[::-1]])
        order = np.argsort(full_x)

        rebuilt_group = pd.DataFrame({
            "run_id": int(run_id),
            output_coordinate: full_x[order],
            value: full_porosity[order],
        })
        if "channelToParticleSizeRatioNp" in group.columns:
            rebuilt_group["channelToParticleSizeRatioNp"] = width
        rebuilt.append(rebuilt_group)

    if not rebuilt:
        raise RuntimeError("No usable wall-to-wall porosity profiles were found.")
    return pd.concat(rebuilt, ignore_index=True), output_coordinate
...
if not wall.empty:
    ...
    wall, wall_coordinate = complete_wall_to_wall_profiles(
        wall, wall_coordinate, "localPorosity"
    )
    x, mean, std, n = aggregate(wall, wall_coordinate, "localPorosity", 120)
    record("wall_to_wall", "positionFromLeftWall/dp", x, mean, std, n)
    ...
    plt.xlabel("Wall-to-wall position from left side wall / characteristic particle diameter [-]")
    plt.ylabel("Local porosity [-]")
    plt.title("Wall-to-wall porosity profile" + np_title_suffix(wall), fontsize=11)
...
if summaries:
    ...
    summary_path = os.path.join(OUT, "zhao_frictionless_porosity_profile_summary.csv")
